...
## sync.Map的函数有哪些？
- Load：从 sync.Map 中加载一个键的值。
- LoadOrStore：从 sync.Map 中加载一个键的值，如果该键不存在，则存储一个新的值。
- Store：存储一个键值对到 sync.Map 中。
- Delete：从 sync.Map 中删除一个键。
- Range：遍历 sync.Map 中的所有键值对。
- LoadAndDelete：从 sync.Map 中加载一个键的值，并删除该键。
- CompareAndSwap：原子地比较并交换一个键的值。
- Swap：原子地交换一个键的值。
```golang
package main
